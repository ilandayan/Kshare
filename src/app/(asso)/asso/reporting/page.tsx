...
import { Gift, CheckCircle, Clock, Heart, TrendingUp } from "lucide-react";
...
export default async function ReportingAssoPage() {
  ...
  const totalQty  = orders?.reduce((sum, o) => sum + (o.quantity ?? 1), 0) ?? 0;

  // Last 30 days
  const thirtyDaysAgo = new Date();
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
  const recent = orders?.filter((o) => new Date(o.created_at) >= thirtyDaysAgo).length ?? 0;
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
        {[
          { label: "Total réservé",     value: total,     icon: Gift,         color: "bg-[#EEF0F8]",  iconColor: "text-[#3744C8]", valueColor: "text-[#3744C8]" },
          ...
          { label: "Repas distribués",  value: totalQty,  icon: Heart,        color: "bg-pink-50",    iconColor: "text-pink-500",  valueColor: "text-pink-500" },
        ].map((kpi) => (
      ...
      <div className="bg-white rounded-2xl border border-[#e2e5f0] shadow-sm p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[#EEF0F8] rounded-xl p-5 text-center">
            <div className="text-3xl font-bold text-[#3744C8] mb-1">{recent}</div>
          ...
          <div className="bg-pink-50 rounded-xl p-5 text-center">
            <div className="text-3xl font-bold text-pink-500 mb-1">{totalQty}</div>
            <div className="text-sm text-gray-500">Repas distribués</div>
          </div>
        </div>
